...
void RunAlgo(size_t n, const vector<size_t> expected_forward) {
  ...
  ASSERT_TRUE(rearrange_wavedec2(n, &forward, &backward));
...
void RunAlgo(size_t n,
  ...
  vector<size_t> backward;
  ASSERT_TRUE(rearrange_wavedec2(n, forward_result, &backward));
  CheckResultPrefix(expected_forward_prefix, *forward_result);
  CheckIsInverse(*forward_result, backward);
}

TEST(RearrangeWavedec2Test, Test1) {
  const size_t f2[] = {0};
  vector<size_t> f(begin(f2), end(f2));
  RunAlgo(1, f);
}

TEST(RearrangeWavedec2Test, Test4) {
  const size_t f2[] = {0, 1, 2, 3};
  vector<size_t> f(begin(f2), end(f2));
  RunAlgo(4, f);
}

TEST(RearrangeWavedec2Test, Test16) {
...
TEST(RearrangeWavedec2Test, Test64) {
  vector<size_t> f(begin(prefix64), end(prefix64));
  RunAlgo(64, f);
}

TEST(RearrangeWavedec2Test, Test256) {
  vector<size_t> f(begin(prefix64), end(prefix64));
  vector<size_t> forward;
...
TEST(RearrangeWavedec2Test, Test4096) {
  vector<size_t> f(begin(prefix64), end(prefix64));
  vector<size_t> forward;
